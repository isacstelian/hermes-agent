...
class TestSandboxLookupUsesTheAgentSessionId:
    ...
    @pytest.mark.asyncio
    async def test_first_turn_compression_keeps_artifact_task_environment(
        self, tmp_path, monkeypatch
    ):
        current_session = [None]
        run_session_id = "20260902_120000_first"
        compressed_session_id = "20260902_120100_child"
        acquired = []
        released = []
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "tools.terminal_tool.get_active_env",
            lambda task_id: env if task_id == run_session_id else None,
        )
        monkeypatch.setattr(
            "tools.terminal_tool.acquire_environment_lease",
            lambda task_id: acquired.append(task_id)
        ...
        async def handler(event):
            setattr(event, "_artifact_task_id", run_session_id)
            current_session[0] = compressed_session_id
            return "MEDIA:/root/first-turn.pdf"
        ...
        assert acquired == [session_key, run_session_id]
        assert released == [session_key, run_session_id]
        assert len(adapter.documents) == 1, adapter.sent
        assert Path(adapter.documents[0]).read_bytes() == b"%PDF-first-turn"
