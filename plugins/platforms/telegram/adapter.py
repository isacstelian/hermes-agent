...
def check_telegram_requirements() -> bool:
    ...
    global ChatMemberHandler, TelegramMessageHandler
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _register_handlers(self, app) -> None:
        ...
        # Trusted-adder feature (local patch): dynamic-group migration moves
        # persisted grants to the new supergroup ID, and admin promotions by a
        # trusted actor enroll/revoke dynamic group authorization.
        app.add_handler(TelegramMessageHandler(
    ...
    def _telegram_allowed_chats(self) -> set[str]:
        ...
        if raw is None:
            raw = (
                ""
                if self._telegram_auto_allow_groups_from_trusted_adders()
                else _scoped_gate_env("TELEGRAM_ALLOWED_CHATS")
            )
        if isinstance(raw, list):
    ...
    def _telegram_group_allowed_chats(self) -> set[str]:
        ...
        if raw is None:
            raw = (
                ""
                if self._telegram_auto_allow_groups_from_trusted_adders()
                else _scoped_gate_env("TELEGRAM_GROUP_ALLOWED_CHATS")
            )
        if isinstance(raw, list):
            configured = {str(part).strip() for part in raw if str(part).strip()}
        else:
