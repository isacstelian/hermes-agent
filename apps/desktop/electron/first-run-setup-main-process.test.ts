...
test('a first-run bootstrap-needed remote apply connects without ensuring or bootstrapping locally', async () => {
  ...
  const cancelSshBootstrap = vi.fn(async (_scope, whileDrained) => whileDrained())
  ...
  assert.equal(cancelSshBootstrap.mock.calls.length, 1)
  assert.equal(cancelSshBootstrap.mock.calls[0][0], '')
  assert.equal(typeof cancelSshBootstrap.mock.calls[0][1], 'function')
